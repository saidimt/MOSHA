...
function fillFieldsCateg(category, id) {
  fill(category, 'category_' + id, 'Category_Type');
  //alert(expiry_date);
  document.getElementById("category_" + id).blur();
}
...
function addPurchase() {
  ...
  if(!notNull(suppliers_name.value, "supplier_name_error"))
    ...
    for(var i = 1; i < row_count; i++) {
      ...
      if(!notNull(medicine_name.value, medicine_name_error.getAttribute('id')))
      ...
      else if((isCategory(packing.value) == "false")){
        document.getElementById(pack_error.getAttribute('id')).style.display = "block";
        document.getElementById(pack_error.getAttribute('id')).innerHTML = "Category not registered!";
        packing.focus();
      }
        ...
        rate_error.innerHTML = "Buy Price(Rate) must be less than SellPrice(MRP)!";
        rate.focus();
      }
...
function addNewPurchase(suppliers_name, invoice_number, payment_type, invoice_date, grand_total) {
  ...
  xhttp.open("GET", "php/add_new_purchase.php?action=add_new_purchase&suppliers_name=" + suppliers_name + "&invoice_number=" + invoice_number + "&payment_type=" + payment_type + "&invoice_date=" + invoice_date + "&invoice_date=" + invoice_date + "&grand_total=" + grand_total, false);
  xhttp.send();
}
