...
function getTotal(id) {
  ...
  total.value = discount <= 100 ? (mrp * qty - (discount * mrp * qty / 100)) : 0;
  ...
  if (net_total >= 0)
    valid_net_total = net_total;
  else
    valid_net_total = 0;
  document.getElementById("total_amount").value = total_amount;
  document.getElementById("total_discount").value = total_discount;
  document.getElementById("net_total").value = valid_net_total;
}
...
function addInvoice() {
    ...
    if(isInvoiceExist(invoice_number.value) == "true")
      ...
      for(var i = 0; i < row_count - 1; i++) {
        ...
        addSale(invoice_number.value, medicines[i].name, medicines[i].batch_id, medicines[i].expiry_date, medicines[i].quantity, medicines[i].mrp, medicines[i].discount, medicines[i].total);
      }
      addNewInvoice(invoice_date.value, total_amount.value, total_discount.value, net_total.value);
  ...
  function addSale(invoice_number, medicine_name, batch_id, expiry_date, quantity, mrp, discount, total) {
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
      if(xhttp.readyState = 4 && xhttp.status == 200)
        xhttp.responseText;
    ...
    xhttp.open("GET", "php/add_new_invoice.php?action=add_sale&invoice_number=" + invoice_number + "&medicine_name=" + medicine_name + "&batch_id=" + batch_id + "&expiry_date=" + expiry_date +  "&quantity=" + quantity + "&mrp=" + mrp + "&discount=" + discount + "&total=" + total, true);
    xhttp.send();
  }

  function addNewInvoice(invoice_date, total_amount, total_discount, net_total) {
    var xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
      if(xhttp.readyState = 4 && xhttp.status == 200)
        document.getElementById("invoice_acknowledgement").innerHTML =  xhttp.responseText;
    };
    xhttp.open("GET", "php/add_new_invoice.php?action=add_new_invoice&invoice_date=" + invoice_date + "&total_amount=" + total_amount + "&total_discount=" + total_discount + "&net_total=" + net_total, true);
    xhttp.send();
  }