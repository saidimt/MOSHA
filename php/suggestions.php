<?php
  ...
  if($con) {
    ...
    if(isset($_GET['action']) && $_GET['action'] == "category")
      showSuggestions($con, "category", "category");
  ...
  function showSuggestions($con, $table, $action) {
    ...
    if($table == "category")
      $query = "SELECT * FROM $table WHERE UPPER(Category_Type) LIKE '%$text%'";
    else
    ...
    while($row = mysqli_fetch_array($result)){
      if($table == "category")
        echo '<input type="button" class="list-group-item list-group-item-action" value="'.$row['Category_Type'].'" style="padding: 5px; outline: none;" onclick="suggestionClick(this.value, \''.$action.'\');">';
      else
